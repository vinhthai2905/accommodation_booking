...
class RoomTypeAvailabilityView(APIView):
    ...
    def _get_booked_room_ids_for_dates(
        self, 
        hotel: KhachSan, 
        booking_date_serializer: BookingDateSerializer
    ) -> set[int]:
        """Return set of room_id which have been booked during requested date."""
        
        overlapping_bookings_hotel: QuerySet[DatPhong] = (
            DatPhong.objects.filter(
                id_hotel=hotel,
        ...
        booked_room_ids_set = set(
            ChiTietDatPhong.objects
            .filter(id_booking__in=overlapping_bookings_hotel)
