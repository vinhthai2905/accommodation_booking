...
export default function ChildrenAgeSearch({ childrenCount, setGuestOptions }) {
        ...
        <div className="flex flex-col gap-3 mt-4">
            <div className="flex flex-wrap gap-2">
                {Array.from({ length: childrenCount }).map((_, index) => (
                    <ChildAgeInput key={`child-age-${index + 1}`} index={index + 1} setGuestOptions={setGuestOptions} />
                ))}
            </div>
