...
export function usePropertyLocation(address, id_ward, wards, setValue, setError, clearErrors) {
    const [isSearching, setIsSearching] = useState(false)

    useEffect(() => {
        if (!address || !id_ward || address.trim().length < 5) {
        ...
        const timeoutId = setTimeout(async () => {
            ...
            if (clearErrors) clearErrors("address") 
            ...
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    if (setError) {
                        setError("address", { 
                            type: "manual", 
                            message: "Không tìm thấy địa chỉ này trên bản đồ. Vui lòng nhập rõ hơn." 
                        })
                    }
                }
            } catch (error) {
    ...
    }, [address, id_ward, wards, setValue, setError, clearErrors])
